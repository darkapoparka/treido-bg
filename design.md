...
| Flows | **97/97** catalogue cards; 424 ordered per-flow screen occurrences in separate per-flow folders; 96 flows complete and one occurrence remains explicitly unresolved: frame 2 of [Marking a review as helpful from Reviews (product detail)](https://mobbin.com/flows/36c23a43-49d3-4932-9595-f122062480b4) because its exact highest-supplied DOM image URL was absent from that page-assets inventory | The missing occurrence was not substituted or deduplicated from another screen. Flow occurrences may repeat the same visual source; hashes report integrity only |
| Recordings / motion | No `<video>` was exposed on the inspected flow pages, including the flow titled Watching a video; 0 flow motion files are therefore claimed. The 16 video records above belong to the screen catalogue | Still-image capture does not prove gesture timing, playback, loading or reduced-motion behavior |
