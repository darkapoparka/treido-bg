...
2026-09-23 checkpoint: after **9a7f8d5**, the latest source batch aligns the Sol welcome, Search Photo tag, Gift result copy/position and reported-review labels. Complete run `20260923-parity-final-implementation` scores **424/424 frames across 97/97 flows**, zero replay/browser errors, **2.528559% mean MAE**, **7.623958% mean bad pixels** and **79 numerical candidates**; it improves **0.004582 MAE points** against the prior full capture. The complete reference journey suite passes **433/433**. Scoped app/test ESLint, web TypeScript, Prettier and diff checks pass. Preserve existing Search/checkout/Orders behavior; detailed evidence and source limits are in the [current checkpoint](shop-implementation-map.md#current-source-checkpoint). No flow is owner accepted.
...
2026-09-23 reference checkpoint: the complete **433/433** browser journey suite passes on the current local source, covering checkout/order loading, cancellation, direct entry/history, delivery actions and responsive controls. The current complete visual run scores **424/424** ordered frames; this UI evidence does not verify real payment, stock or fulfillment. The earlier **2c74021** isolated production build and **136/136** units remain tied to that source commit. Remaining visual source work stays in Tasks 3/6; [current evidence](docs/STATUS.md) records exact scope.
...
2026-09-23 checkpoint: the [frame ledger](shop-frame-ledger.md) now records `20260923-parity-final-implementation`: **424/424** frames across **97/97** flows, zero replay/browser errors, **79 numerical candidates**, **345 above at least one threshold**, and **2.528559% mean MAE**. The full current-source browser suite passes **433/433**. The standalone visual pairs and exact accessible limits are in the [checkpoint](shop-implementation-map.md#current-source-checkpoint). Three pinned original-media failures, exact Chemical/Pura motion and the obscured eighth notification title remain open; latest Mobbin access requires a paid plan. Source approval stays **0/97**. The earlier **2c74021** production build, 50-frame production comparison and unit evidence remain tied to that source. See also the [resume prompt](single-session-execution.md#new-session-prompt).
