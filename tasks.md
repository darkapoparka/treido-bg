...
2026-09-23 checkpoint: **fd34bc7** aligns Kitsch collections/recommendations and store reviews, corrects source-panel clipping, preserves explicit PDP rating snapshots and protects Saved persistence from oversized collection names. **132bf51** retains the Bag's added-state button color. The latest complete capture scores **424/424**, zero browser/replay errors, **2.586946% mean MAE**; later **26/26** affected frames show no regression. Units pass **134/134**. The full browser run is **427/428**, followed by **32/32** passing siblings after fixing the failed test's pre-hydration snapshot. Exact source, commands and limitations are in the [current checkpoint](shop-implementation-map.md#current-source-checkpoint). Continue remaining shared typography/media/motion differences; all 97 flows remain in scope and none is owner accepted.
...
2026-09-23 reference checkpoint: Receipt payment information, source-specific checkout/order recommendation media, connected returns, checkout fields and reversible collection-stage history are implemented. The current batch protects collection persistence and distinguishes Bag purchase-button states. Full reference journeys and the scoped failure rerun are recorded in [current evidence](docs/STATUS.md); they do not verify real payment, stock or fulfillment. Remaining reference typography/media/motion work stays in Tasks 3/6.
...
2026-09-23 checkpoint: the [frame ledger](shop-frame-ledger.md) is regenerated from tracked-clean **fd34bc7** / **20260923-finalization-full**: **424/424** frames, zero replay/browser errors, **72 numerical candidates**, **352 above at least one threshold**. Follow-up **132bf51** has a **26/26** affected-frame capture; it is not mislabeled as a new complete run. All **755** frozen source hashes verify; original-media verification still fails three assets and exact motion remains unresolved. Full browser coverage initially passes **427/428**, then the exact hydration-race failure and 31 siblings pass; **134/134** units pass. No fresh production build or real-service qualification was run. Source approval remains **0/97**. Exact evidence and next work are in the [implementation checkpoint](shop-implementation-map.md#current-source-checkpoint) and [new-session prompt](single-session-execution.md#new-session-prompt).
