...
2026-09-23 checkpoint: source commit **2302ad1** fits the Store on-sale sort row and test, guards the Support Chat capture, mutes root Explore shelf headings, and outlines the Get the Look result titles. Run `20260923-continuation-full-source-audit` scores **424/424 frames across 97/97 flows**, zero replay/browser errors, **2.522760% mean MAE**, **7.614299% mean bad pixels** and **80 numerical candidates**. Ranked direct-pair checks cover residual product, Search/photo, order, review, checkout, Saved, collection, Mini and Explore states. Web TypeScript, scoped ESLint, Prettier and diff checks pass; source acceptance remains **0/97**. See the [current checkpoint](shop-implementation-map.md#current-source-checkpoint) for comparisons and source limits.
...
2026-09-23 reference checkpoint: the complete **434/434** browser journey suite passes across **76 files** on the current source, including checkout/order loading, cancellation, direct entry/history, delivery actions, responsive controls, and the expanded Store-filter cancel/Back/Forward/Done/reload path. The complete visual run scores **424/424** ordered frames; this UI evidence does not verify real payment, stock or fulfillment. Earlier **2c74021** isolated production build and **136/136** units remain tied to that source commit. Remaining visual source work stays in Tasks 3/6; [current evidence](docs/STATUS.md) records exact scope.
...
2026-09-23 checkpoint: the [frame ledger](shop-frame-ledger.md) records `20260923-continuation-full-source-audit`: **424/424** frames across **97/97** flows, zero replay/browser errors, **80 numerical candidates**, **344 above at least one threshold**, and **2.522760% mean MAE**. The current browser suite passes **434/434**. Source/live pairs and the precise remaining limits are in the [checkpoint](shop-implementation-map.md#current-source-checkpoint). Three pinned original-media failures, exact Chemical/Pura motion and the dock-obscured eighth notification title remain open; latest Mobbin access requires a paid plan. Source approval stays **0/97**. Earlier production build and unit evidence remain tied to their source commits. See also the [resume prompt](single-session-execution.md#new-session-prompt).
