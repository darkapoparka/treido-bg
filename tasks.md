...
2026-09-23 reference checkpoint: the complete **434/434** browser journey suite passes across **76 files** on commit 1042422, including checkout/order loading, cancellation, direct entry/history, delivery actions, responsive controls, and the expanded Store-filter path. Final source commit **aff7dd8** changes only the photo-answer top fade mask; its Search/photo sibling suite passes **18/18** at the documented mobile widths. Complete run `20260923-final-parity-verified` scores **424/424** ordered frames with zero replay errors. This UI evidence does not verify real payment, stock or fulfillment. Earlier **2c74021** isolated production build and **136/136** units remain tied to that source commit. See [current evidence](docs/STATUS.md); source acceptance remains open.
...
2026-09-23 checkpoint: the [frame ledger](shop-frame-ledger.md) records `20260923-final-parity-verified`: **424/424** frames across **97/97** flows, zero replay/browser errors, **80 numerical candidates**, **344 above at least one threshold**, **2.521206% mean MAE** and **7.604800% mean bad pixels**. Browser coverage passes **434/434** on 1042422; the final photo-tail CSS regression passes **18/18**. The ranked source/live audit addressed the tracking bottom band, captured photo-step copy and clipped photo-tail price; remaining exact media, motion, hidden notification copy, and system-font/platform differences are itemized in the [checkpoint](shop-implementation-map.md#current-source-checkpoint). Mobbin access requires a paid plan. Source approval stays **0/97**. Earlier production build and unit evidence remain tied to their source commits. See also the [resume prompt](single-session-execution.md#new-session-prompt).
