...
async function openScenario(
  ...
  if (!baseURL) throw new Error("Reference tests require the configured preview URL");
  await page.context().addCookies([
    { name: "shop-reference-scenario", value: scenario, url: baseURL, httpOnly: true, sameSite: "Lax" },
  ]);
  await page.goto(path);
  await expect(page.locator("html")).toHaveAttribute("data-reference-scenario", scenario);
  await expect(page.locator('[data-shop-interactive="true"]').first()).toBeAttached();
}
const card = (page: Page, id: string) => page.locator(`.saved-grid [data-product-id="${id}"]`);
const button = (page: Page, name: string) => page.getByRole("button", { name, exact: true });
async function openCollection(page: Page) {
  await button(page, "Private Favs").click();
  await expect(page.getByRole("heading", { name: "Favs", exact: true })).toBeVisible();
}

test("Saved renders the captured multi-brand library, price, variant and promotion", async ({ page, baseURL }) => {
  ...
  await expect(card(page, "rhode-pink-captured").locator(":scope > b")).toHaveCount(0);
...
test("an obscured Saved listing never receives an invented price or purchase action", async ({ page, baseURL }) => {
  await openScenario(page, baseURL, "saved-library");
  const trigger = button(page, "View captured Pink rhode tube");
  await trigger.click();
  const dialog = page.getByRole("dialog", { name: "Captured item details", exact: true });
  await expect(dialog).toContainText("listing name, price and lower photograph are obscured");
  await expect(dialog.getByRole("button", { name: /buy|pay|checkout/i })).toHaveCount(0);
...
test("collection creation rejects whitespace and creates exactly one collection", async ({ page, baseURL }) => {
...
test("every More ideas card selects a real item, including Jojoba at the captured price", async ({ page, baseURL }) => {
  await openScenario(page, baseURL, "saved-collection");
  await openCollection(page);
  await button(page, "Find more ideas").click();
  await expect(page.locator(".saved-grid > article")).toHaveCount(6);
  const ids = ["idea-rice-wash", "idea-rosemary-bar", "idea-rosemary-bundle", "idea-purple-bundle", "idea-rosemary-liquid", "idea-jojoba"];
  for (const id of ids) {
  ...
  await expect(page.getByRole("heading", { name: "Jojoba Bead Exfoliating Body Wash Bar", exact: true })).toBeVisible();
});

test("removing a recommendation from a collection does not unsave the product", async ({ page, baseURL }) => {
  await openScenario(page, baseURL, "saved-collection");
  await openCollection(page);
  await button(page, "Find more ideas").click();
...
test("collaboration dismissal survives a real product navigation and return", async ({ page, baseURL }) => {
  await openScenario(page, baseURL, "saved-collection");
  await openCollection(page);
...
test("More ideas Back, Forward and Done preserve collection scroll and focus", async ({ page, baseURL }) => {
  await openScenario(page, baseURL, "saved-collection");
  await openCollection(page);
  ...
  await ideas.click();
  await expect(page.getByRole("heading", { name: "More ideas", exact: true })).toBeVisible();
  await page.goBack();
  await expect(page).toHaveURL(/\/saved\?collection=source-favs$/);
...
test("public and private states match the captured controls without publishing or sending invitations", async ({ page, baseURL }) => {
  await openScenario(page, baseURL, "saved-collection-edited", "/saved?collection=source-favs");
  await button(page, "Collection options").click();
  await button(page, "Make collection public").click();
  const dialog = page.getByRole("dialog");
  await expect(dialog).toContainText("Your collection will be discoverable by others and may appear on the feed.");
  await expect(button(page, "Make public")).toHaveAttribute("aria-describedby", "collection-public-boundary");
...
test("editing Cancel preserves the name and a later Save commits the emoji name", async ({ page, baseURL }) => {
  await openScenario(page, baseURL, "saved-collection-expanded", "/saved?collection=source-favs");
  await button(page, "Collection options").click();
...
test("expanded collection deletion retains all saved products, not only the final capture's pair", async ({ page, baseURL }) => {
