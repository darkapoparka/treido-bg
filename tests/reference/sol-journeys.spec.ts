...
const phase = (page: Page, value: string) => page.locator(`[data-sol-phase="${value}"]`);
...
async function capture(page: Page, id: string) {
  ...
  const broken = await page.locator("img:visible").evaluateAll((elements) =>
    elements.filter((element) => !(element as HTMLImageElement).naturalWidth).map((element) => element.getAttribute("src")),
  );
  expect(broken, `${id} must not capture broken artwork`).toEqual([]);
  await mkdir(".qa/shop-parity/media-review", { recursive: true });
  await page.screenshot({ path: `.qa/shop-parity/media-review/${id}-live.png` });
}
...
async function connect(page: Page) {
  ...
  await page.getByRole("dialog", { name: "Allow access to your microphone?", exact: true })
    .getByRole("button", { name: "Share", exact: true }).click();
  await expect(phase(page, "greeting")).toBeVisible();
}

test("Sol setup follows catalogue, consent, welcome, permission, connecting and greeting without a provider request", async ({ page }) => {
  ...
  await access.getByRole("button", { name: "Agree", exact: true }).click();
  await expect(access).not.toBeVisible();
  await expect(page.getByRole("heading", { name: "Hi, I’m Sol", exact: true })).toBeVisible();
  await capture(page, "f053-003");
  await button(page, "Allow & Continue ›").click();
  const microphone = page.getByRole("dialog", { name: "Allow access to your microphone?", exact: true });
  ...
  await expect(page.getByRole("heading", { name: normalGreeting, exact: true })).toBeVisible();
  await capture(page, "f053-006");
  expect(outside).toEqual([]);
});

test("the captured sunglasses conversation reaches the correct two choices and real AKIRA result, not another product", async ({ page }) => {
  await page.setViewportSize({ width: 393, height: 793 });
  await enter(page);
  await connect(page);
  ...
  await expect(button(page, "Gold rimless glasses")).toHaveAttribute("aria-pressed", "true");
  await expect(button(page, "Dark sunglasses")).toHaveCount(0);
  await capture(page, "f054-004");
  const results = page.getByRole("region", { name: "Captured sunglasses recommendations", exact: true });
  await expect(results).toBeVisible();
  await expect(results.locator('.product-media a[href="/products/u-see-me"]')).toBeVisible();
  await expect(results).toContainText("AKIRA");
  await expect(results).toContainText("$14.90");
  await expect(results.locator('[data-product-id="hush-glasses"]')).toHaveCount(1);
  await expect(results.locator('a[href="/products/round-sunglasses"]')).toHaveCount(0);
  await capture(page, "f054-005");
  const save = results.getByRole("button", { name: /^Save U SEE ME GLASSES$/i });
  await save.click();
  await expect(results.getByRole("button", { name: /^Unsave U SEE ME GLASSES$/i })).toHaveAttribute("aria-pressed", "true");
  await results.getByRole("button", { name: "Save HUSH", exact: true }).click();
  await page.getByRole("link", { name: "Close Sol: Browse by Voice", exact: true }).click();
  await page.getByRole("link", { name: "Home", exact: true }).click();
  await page.getByRole("link", { name: "Saved", exact: true }).click();
  await expect(page.locator('.saved-grid [data-product-id="u-see-me"]')).toBeVisible();
  ...
  await hush.getByRole("button", { name: "View captured HUSH", exact: true }).click();
  await expect(page.getByRole("dialog", { name: "Captured item details", exact: true })).toContainText("FORK Eyewear");
});

test("mute, text drafting, baseball-cap choices and browser history retain their own source state", async ({ page }) => {
  await page.setViewportSize({ width: 393, height: 793 });
  await enter(page);
  await connect(page);
  await capture(page, "f056-001");
  await button(page, "Mute microphone preview").click();
  await expect(button(page, "Unmute microphone preview")).toHaveAttribute("aria-pressed", "true");
  await expect(page.getByRole("heading", { name: "Hey Alex, I’m Sol. What are we hunting for today?", exact: true })).toBeVisible();
  ...
  await expect(phase(page, "choices")).toHaveAttribute("data-sol-topic", "caps");
  ...
  await expect(phase(page, "selected")).toHaveAttribute("data-sol-topic", "caps");
  await expect(phase(page, "selected").getByRole("status")).toContainText("Boston baseball cap selected");
  await expect(page.getByRole("region", { name: "Captured sunglasses recommendations" })).toHaveCount(0);
  await page.goBack();
  await expect(phase(page, "choices")).toBeVisible();
  await page.goBack();
  await expect(phase(page, "greeting")).toBeVisible();
...
test("cancelling microphone setup restores focus and Back cancels the pending local connection", async ({ page }) => {
  await enter(page);
  await button(page, "Allow & Continue ›").click();
...
test("Sol contains its editable composer at sibling widths and keeps unsupported queries as drafts", async ({ page }) => {
  ...
  for (const width of [320, 393, 430]) {
    ...
    const contained = await input.evaluate((element) => {
      ...
      return box.left >= 0 && box.right <= innerWidth && box.bottom <= innerHeight && document.documentElement.scrollWidth <= innerWidth;
    });
    expect(contained, `Sol at ${width}`).toBe(true);
  }
  await input.press("Enter");
  const unavailable = page.getByRole("dialog", { name: "Recorded responses", exact: true });
