...
The returning-Home batch over **19f9857** uses frozen flow `b5716e20-b094-463c-b74b-a5e983dd1651/009.webp`: exposed campaign photo **[17,271,359,493]** and white brand mark **[38,297,153,22]**. `media.server.ts` removes the captured heading/control ink and all six product cards. Its opt-in horizontal fill interpolates only known left/right background colors beneath removed cards; it does not reconstruct hidden products or controls. This prevents fixed cutout shapes from appearing when the live grid resizes. White-mark extraction removes the rectangular photographic background. Existing assets retain their previous processing.

`home-campaigns.tsx` selects those two assets only for returning DRMTLGY's grid. `home-campaigns.css` restores the 15px stack margin, #f0f0f0 cards, 153px brand width and measured header offsets, 168px three-link dock, and a 40px photo-edge blend. At narrow widths the email prompt wraps without covering its arrow. All product navigation, Save controls and campaign menus remain live DOM. Browser inspection at 320/393/430 confirms the responsive background and prompt; Retinol opens its canonical product and Back restores its originating control's focus. Independent read-only agents checked scope, source geometry and media processing.

Focused run **20260920-returning-home-verified** captures flows 2/3/42/94 **21/21**, zero replay/browser errors, with no >0.15-point regression against `20260920-minis-qualified-mobile`. Final returning-only capture **20260920-returning-clean-header** improves f094-009 **5.538433 -> 4.170% MAE**; `.qa/shop-parity/20260920-returning-home-comparison.json` records the comparison. The 21-state run precedes the final responsive backdrop, wordmark and narrow prompt finishing; those changes are returning-only. The full ledger remains the last complete af59e39 run.

`home-overlays.spec.ts`, `home-discovery-continuity.spec.ts` and `home-continue-journey.spec.ts` pass **20/20** (`20260920-returning-home-journeys.json`). The final strengthened returning test verifies all campaign images decode, Save works and email text/arrow remain within the button at 320/430: **1/1** (`20260920-returning-responsive-journey.json`). TypeScript, scoped ESLint and formatting pass; the first root-config lint invocation failed to resolve the Next rule and was corrected to the application's config. No production build, complete capture or unit rerun is claimed for this batch. Product-photo differences and source typography remain unresolved. Push of 19f9857 is waiting at the existing Git credential-helper selector; no credential setting was changed or duplicate push started.
