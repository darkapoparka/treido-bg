...
Next source-grounded probes from the read-only assistant review: a dedicated rear-cap photo-tag crop/ring without changing shared `assistant-cap`, photo recommendation `strong/b` weight 500 -> 600, and rating gap 6 -> 2px. Owners are `assistant.tsx`, `search-entry.module.css` and `media.server.ts`. Large photographs and primary layout already align; the prose band contributes about **2.316 whole-frame MAE points** in f045-005/006, and translating it does not improve alignment. Photo-answer and Shea-description typography remain open after rejected font/blur probes. Verify any retained change against flows 45/46/47.
