...
The Mini refinement extends **88e38df**, owned by `discovery/minis.tsx` and `minis.module.css`. Skin's measured heading/action rhythm and CSS gradient improve its welcome, chooser and analysis states. Get the Look's all-matches action opens Blazer at y70, transfers keyboard focus to the heading and restores selected-piece focus/scroll on Back. Its recipe now observes the real transition instead of forcing the destination scroll. The sticky header retains the source's rounded content corners. Existing product links, Save state, bounded unknown photographs and local-photo boundaries remain connected.

**20260920-minis-real-navigation** records **26/26** Skin/Look/Gift frames; **20260920-look-focus-verified** records the final **8/8** Look frames. Both have zero replay/browser errors and no >0.15-point regression. Compared with `20260919-assistant-final-mobile`, f057-002 improves **5.277 -> 1.892% MAE**, f057-004 **4.268 -> 2.549%**, f057-005 **3.738 -> 2.041%**, f058-008 **4.515 -> 4.304%**. Browser verification used 320/393/430 widths on 6412. Focused Mini tests and lint/formatting pass; the isolated `.next-parity` production build passes TypeScript and generates 39 pages, and units pass **106/106**. The corrected full production qualification is pending; [current status](docs/STATUS.md#active-repair-checkpoint) records the stopped wrong-working-directory attempt and updated source-grounded auxiliary assertion. These partial captures do not replace the complete ledger or establish 1:1 acceptance.

The read-only subagent review also identified the next visible collection issue: f014-003's saved-product artwork is narrower and shifted relative to source, and Sort by uses a right-facing rather than downward chevron. Inspect the complete owning/sibling family before fitting `first-collection.css` and `StoreCriteria`; do not treat the recommendation as an implemented fix. Existing photo-answer and description typography differences remain after prior rejected font/blur probes.
...
The September 20 photo-Assistant correction extends **347b7d1**: inline comparison actions wrap without invented destinations, three bounded structured-rail photographs replace an unsupported identity, and comparison title/shadow treatment follows the frozen source. Existing DOM product links, Save state, editing, disclosure, composer and Jeans/Feedback siblings are preserved. Canonical owners are discovery/assistant.tsx, search-entry.module.css, search-photo.module.css and catalog/reference/media.server.ts.

Focused run **20260920-photo-final** scores **16/16** flow-45/46/47 states with no replay/browser errors or >0.15-point regression against the complete baseline. f045-005: 6.059%; f045-006: 5.487%; f045-007: 6.002%; f045-008: 4.614%. Search/photo/answer/Sheet journeys pass **22/22**, including Space/Back/focus and the three bounded fragments; units pass **106/106**, web lint and TypeScript pass. The earlier cold-route timeout and rejected font/blur probes are recorded in [current status](docs/STATUS.md), not presented as successful evidence.

The complete ledger remains sourced from **20260919-assistant-final-mobile** at application **5664953**: **424/424**, **3.146873% mean MAE**, **9.829613% mean bad pixels**, **41 numerical candidates**, and **279/279 Windows production journeys**. The later focused run is not a replacement full-app baseline. The 5664953 macOS run finished 274/279 dev-server journeys with five failures; current status owns exact failures and the next product/CI continuation. **Source acceptance remains 0/97, not 1:1.**
