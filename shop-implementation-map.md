...
The read-only subagent review identifies the next correction precisely: f014-003's **inner photograph**, not its entire card stack, is too small and shifted right/down. Start with the existing `first-collection.css` image owner; keep sheet bounds and the near-matching decorative stack. In the collection variant, both Sort by and Price chevrons point downward in f014-002; search already rotates them correctly, and the f016 filter-sheet rows must remain right-facing. Compare f014-002/003, f016-001..004 and f040-004 and run real first-save creation/dismissal and sibling-width checks. This recommendation is not an implemented fix. Photo-answer and Shea-description typography remain open after the documented rejected font/blur probes.
